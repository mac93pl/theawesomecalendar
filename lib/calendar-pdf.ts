...
import { CalendarMonth, CalendarStyle, calendarFileName, createMonths } from '@/lib/calendar';
...
function drawCropMarks(page: PDFPage) {
  ...
  page.drawLine({
    start: { x: 24, y: height / 2 }, end: { x: width - 24, y: height / 2 },
    thickness: 0.55, color: GRAY, dashArray: [4, 4],
  });
}
...
function drawMonth(page: PDFPage, data: CalendarMonth, x: number, top: number, width: number, style: CalendarStyle, fonts: FontPair) {
  ...
  for (let index = 0; index < data.days; index += 1) {
    ...
    page.drawLine({
      start: { x: dayX, y: timelineY + 17 }, end: { x: dayX, y: timelineY + 69 },
      thickness: 0.35, color: LIGHT_GRAY,
    });

    if (style === 'rice') {
      page.drawEllipse({
        x: dayX, y: timelineY + 15, xScale: weekend ? 2.1 : 1.3, yScale: weekend ? 6 : 4.2,
        borderWidth: 0.65, borderColor: INK, color: weekend ? INK : undefined,
      });
    } else {
      const markWidth = Math.max(step - 1.5, 2.8);
      page.drawRectangle({
        x: dayX - markWidth / 2, y: timelineY + 9, width: markWidth, height: 11,
        borderWidth: 0.65, borderColor: INK, color: weekend ? INK : undefined,
      });
    }
    ...
    page.drawText(number, {
      ...
      y: timelineY, size, font: fonts.body, color: GRAY,
    });
  }
  page.drawText(data.label, { x, y: timelineY - 24, size: 13, font: fonts.display, color: INK });
}

function drawStrip(page: PDFPage, months: CalendarMonth[], row: number, style: CalendarStyle, fonts: FontPair, stripNumber: number) {
  ...
  const strip = 'PASEK ' + String(stripNumber).padStart(2, '0');
  page.drawText(strip, {
    x: pageWidth - marginX - fonts.body.widthOfTextAtSize(strip, 7),
    y: footerY, size: 7, font: fonts.body, color: INK,
  });
  page.drawRectangle({ x: pageWidth / 2 - 22, y: footerY - 2, width: 44, height: 2, color: YELLOW });
}

export async function generateCalendarPdf(start: string, end: string, style: CalendarStyle) {
  const months = createMonths(start, end);
  const displayBytes = await fetch('/fonts/oswald-latin-ext.ttf').then((response) => {
    if (!response.ok) throw new Error('Nie udało się wczytać fontu Oswald.');
  ...
  document.setSubject('Kalendarz liniowy ' + start + '–' + end);
  ...
  for (let pageIndex = 0; pageIndex < Math.ceil(months.length / 4); pageIndex += 1) {
    ...
    drawCropMarks(page);
    drawStrip(page, pageMonths.slice(0, 2), 0, style, fonts, pageIndex * 2 + 1);
    if (pageMonths.length > 2) {
      drawStrip(page, pageMonths.slice(2, 4), 1, style, fonts, pageIndex * 2 + 2);
    }
    page.drawText('DRUKUJ 100%  /  WYTNJ WZDLUZ LINII  /  SKLEJ PASKI W KOLEJNOSCI', {
      x: 38, y: 6, size: 4.8, font: fonts.body, color: GRAY,
    });
  }

  return {
    bytes: await document.save(),
    filename: calendarFileName(start, end, style),
